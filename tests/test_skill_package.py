...
def test_skill_guardrails_cover_adversarial_requirements():
    ...
    tests = json.loads((ROOT / "skills" / "adversarial-tests.json").read_text())
    assert len(tests["cases"]) >= 7


def test_host_configs_are_local_stdio_only():
    combined = (ROOT / "host-integrations" / "claude-desktop.json").read_text() + (ROOT / "host-integrations" / "codex.toml").read_text()
