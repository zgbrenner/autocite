    ...
    if item.correction_level == "unsupported":
        lines.extend(
            (
                "",
                (
                    "This item remains in the audit report even when marked "
                    "reviewed, because it is outside AutoCite's current rule "
                    "coverage."
                ),
            )
        )
        ...
        lines.extend(
            (
                "",
                (
                    "Accept marks this review item resolved. Reject or Pending "
                    "keeps it in the exported review record."
                ),
            )
        )
    return "\n".join(lines)
...
def qt_cursor_position(text: str, python_index: int) -> int:
    """Convert a Python code-point offset to Qt's UTF-16 cursor position."""

    bounded = max(0, min(int(python_index), len(text)))
    return len(text[:bounded].encode("utf-16-le")) // 2
...
def run_review_workspace(argv: Sequence[str] | None = None) -> int:
    ...
    class Window(QMainWindow):
        def __init__(self) -> None:
            ...
            heading = QLabel(
                "<h1>AutoCite</h1>"
                "<p>Review legal citations locally, inspect every change, and "
                "keep the source file unchanged.</p>"
            )
            heading.setWordWrap(True)
            layout.addWidget(heading)

            self.drop_hint = QLabel(
                "Drop a TXT, Markdown, DOCX, or searchable PDF here, or choose "
                "a document below."
            )
            ...
            self.review_button.setAccessibleName(
                "Review the selected document again"
            )
            ...
            self.mode.addItem(
                "Bluepages: court and practice documents",
                "bluepages",
            )
            self.mode.addItem(
                "Whitepages: academic legal writing",
                "whitepages",
            )
            ...
            if not _local_model_runtime_available():
                ...
                if model_item is not None:
                    ...
                    model_item.setToolTip(
                        "The optional model runtime is not included in the "
                        "portable build."
                    )
            ...
            self.summary_label = QLabel(
                "Ready. Standard review works offline and is recommended for "
                "8 GB computers."
            )
            ...
            self.decision_filter.setAccessibleName(
                "Review item status filter"
            )
            ...
            self.source_filter.setAccessibleName(
                "Citation source type filter"
            )
            ...
            self.rule_filter.addItem("All rule families", None)
            self.rule_filter.setAccessibleName("Rule family filter")
            self.rule_filter.currentIndexChanged.connect(self.apply_filters)
            ...
            for button, name in (
                (
                    self.accept_button,
                    "Accept the selected change or mark the selected item reviewed",
                ),
                (
                    self.reject_button,
                    "Reject the selected review item",
                ),
                (
                    self.reset_button,
                    "Reset the selected item to pending review",
                ),
                (
                    self.accept_all_button,
                    "Accept every high-confidence safe mechanical change",
                ),
                (
                    self.previous_button,
                    "Select the previous visible review item",
                ),
                (
                    self.next_button,
                    "Select the next visible review item",
                ),
            ...
            self.corrected = QPlainTextEdit()
            self.corrected.setReadOnly(True)
            self.corrected.setPlaceholderText(
                "The corrected preview will reflect the changes you currently "
                "accept."
            )
            self.corrected.setAccessibleName("Corrected document preview")
            self.original = QPlainTextEdit()
            self.original.setReadOnly(True)
            self.original.setPlaceholderText(
                "The extracted original text will appear here."
            )
            self.original.setAccessibleName("Original document text")
            self.details = QTextBrowser()
            self.details.setPlaceholderText(
                "Select a review item to inspect its explanation, confidence, "
                "source range, and provenance."
            )
            ...
            for button, name in (
                (
                    self.copy_button,
                    "Copy the corrected preview to the clipboard",
                ),
                (
                    self.save_button,
                    "Save a reviewed Word document",
                ),
                (
                    self.report_button,
            ...
            boundary = QLabel(
                "AutoCite checks supported citation mechanics. It does not "
                "determine good-law status, legal support, controlling "
                "authority, or compliance with every local rule."
            )
            ...
            self._previous_shortcut = QShortcut(
        ...
        def _save_settings(self) -> None:
            ...
            self._settings.setValue(
                ...
                self.doc_type.currentData(),
            )
            self._settings.setValue(
                "choices/jurisdiction",
                self.jurisdiction.currentData(),
            )
        ...
        def start_review(self) -> None:
            ...
            options = {
                "document_type": str(
                    self.doc_type.currentData() or "auto"
                ),
        ...
        def load_result(
            ...
            if state.review_session is None:
                raise RuntimeError(
                    "completed desktop review has no review session"
                )
            ...
            if warning:
                self.statusBar().showMessage(
                    "Review complete | DOCX preservation unavailable | "
                    "source unchanged"
                )
                QMessageBox.warning(
                    self,
                    "DOCX preservation unavailable",
                    warning,
                )
            else:
                self.statusBar().showMessage(
        ...
        def apply_filters(self) -> None:
            ...
            self.model.set_filter(
                str(self.decision_filter.currentData() or "all")
            )
            self.model.set_severity(
                self.severity_filter.currentData()
            )
            self.model.set_source_type(
                self.source_filter.currentData()
            )
            self.model.set_rule(self.rule_filter.currentData())
        ...
        def refresh_workspace(self) -> None:
            if (
                ...
                return
            try:
                preview = self.model.render_corrected_text(
            ...
            self.summary_label.setText(
                ...
                f"{workspace.accepted} accepted | "
                f"{workspace.pending} pending | "
                f"{workspace.rejected} rejected | "
                f"{workspace.unsupported} outside coverage."
            )

            current = self.model.current_item
            current_id = current.item_id if current is not None else None
        ...
        def _highlight_original(self, item: ReviewItem) -> None:
            ...
            start = qt_cursor_position(text, item.start)
            end = qt_cursor_position(text, item.end)
        ...
        def _update_item_buttons(self) -> None:
            ...
            self.undo_button.setEnabled(
                bool(has_model and self.model.can_undo)
            )
            self.redo_button.setEnabled(
                bool(has_model and self.model.can_redo)
            )
        ...
        def _confirm_unresolved(self) -> bool:
            ...
            answer = QMessageBox.question(
                ...
                (
                    f"{summary.pending} item(s) remain pending and "
                    f"{summary.unsupported} item(s) are outside current "
                    "coverage. Export them in the review record anyway?"
                ),
                (
        ...
        def save_docx(self) -> None:
            ...
            try:
                ...
                return
            mode = str(
                metadata.get("preservation_mode")
                or "reviewed_text"
            )
            self.statusBar().showMessage(
                f"Saved reviewed document using {mode}: "
                f"{metadata['path']}"
            )
        ...
        def show_final_review(self) -> None:
            ...
            preservation = (
                ...
                if (
                    ...
                    and self.state.source_bytes is not None
                )
                else (
                    "A text-level reviewed DOCX will be produced for this "
                    "source format."
                )
            )
            QMessageBox.information(
                ...
                "\n".join(
                    (
                        ...
                        (
                            "AutoCite has not determined good-law status or "
                            "legal support."
                        ),
                    )
                ),
        ...
        def closeEvent(self, event: QCloseEvent) -> None:
            ...
            if (
                ...
                answer = QMessageBox.question(
                    ...
                    "Review in progress",
                    (
                        "AutoCite is still reviewing this document. Close "
    ...
    if existing is not None:
        application._autocite_window = window
        return 0
